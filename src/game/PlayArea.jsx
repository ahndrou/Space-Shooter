...
const WALL_ORIENTATIONS = new Map(
  Object.entries({
    TOP: 1,
    BOTTOM: 2,
    RIGHT: 3,
...
export default function PlayArea({ size }) {
  return [...WALL_ORIENTATIONS].map(([, orientation]) => (
    <BoundaryWall key={orientation} orientation={orientation} size={size} />
  ));
}
...
function BoundaryWall({ orientation, size }) {
  ...
  const customUniformsRef = useRef({
    uCollisionPoints: {
      value: Array.from({ length: MAX_COLLISIONS }, () => new Vector2(-1, -1)),
    },
    uCollisionTimers: {
      value: Array.from({ length: MAX_COLLISIONS }, () => 0),
    },
  });
  ...
  function handleCollision(collision) {
    ...
    let collosionPointVec = new Vector3(
      collisionPoint.x,
      collisionPoint.y,
      collisionPoint.z,
    );
    ...
    collisionPoint
      .divideScalar(size / 2)
      .addScalar(1)
      .divideScalar(2); // Range 0 <> 1.

    customUniformsRef.current.uCollisionPoints.value[
      collisionIndex.current
    ].set(collisionPoint.x, collisionPoint.y);
