...
function createInitialPositions(playAreaSize, enemySize, enemyCount) {
  ...
  for (let i = 0; i < enemyCount; i++) {
    ...
    if (attempts === 100) {
      throw new Error(
        `Could not find empty cell for enemy. Play area is likely too small for given combination of enemy size and count.`,
      );
    }
  }
...
function useSpawnEnemy(enemySize, playAreaSize) {
  ...
  function findSpawnPosition() {
    ...
    while (attempts < 100) {
      ...
      world.intersectionsWithShape(shapePosition, shapeRotation, shape, () => {
        hit = true;
        return false;
      });
    ...
    throw new Error(
      "Could not find a non-intersecting spawn position for new enemy.",
    );
  }
...
export default function Level({ playAreaSize, spaceshipRb }) {
  const [enemies, setEnemies] = useState(() =>
    createInitialEnemyState(playAreaSize, ENEMY_SIZE, BASIC_ENEMY_COUNT),
  );
  const [snakes, setSnakes] = useState(() =>
    createInitialEnemyState(playAreaSize, 0, SNAKE_COUNT),
  );
  const [explodingEnemies, setExplodingEnemies] = useState(() =>
    createInitialEnemyState(playAreaSize, 0, EXPLODING_ENEMY_COUNT),
  );
  const [collectables, setCollectables] = useState(() =>
    createInitialEnemyState(playAreaSize, 0, COLLECTABLES_COUNT),
  );
  ...
  const removeExplodingEnemy = useCallback(
    (enemyId) => {
      setExplodingEnemies((enemies) =>
        enemies.filter((enemy) => enemy.id !== enemyId),
      );
    },
    [setExplodingEnemies],
  );

  const removeSnakeEnemy = useCallback(
    (enemyId) => {
  ...
  useFrame((state) => {
    if (state.clock.elapsedTime - lastEnemyTime.current > 10) {
      addEnemy(
        enemySetters[Math.round(Math.random() * (enemySetters.length - 1))],
      );
